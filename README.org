...
- =--tags zsh=: Limits the execution to the specified role.
...
The playbook will use =become= to switch to the administrative user (root) for package installation and the specified =zsh_user= for home directory configuration.
